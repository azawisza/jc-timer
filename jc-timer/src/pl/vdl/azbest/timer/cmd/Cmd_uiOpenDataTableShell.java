...
import pl.vdl.azbest.timer.gui.Shell_DataTable;

public class Cmd_uiOpenDataTableShell implements Command {

	public void execute() {
		Conf.getInstance().getDisplay().syncExec(new Runnable() {
			public void run() {
				Shell_DataTable.getInstance().openSWTWidgetSingleton(
						Conf.getInstance().isDataTableShellOpen());
			}
		});
