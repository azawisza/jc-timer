...
import pl.vdl.azbest.timer.clipcap.ImageCapture;
import pl.vdl.azbest.timer.gui.GUIFacade;

public class Cmd_CaptureClipboardToFile implements Command {

	public void execute() {
		Conf.getInstance().getDisplay().syncExec(new Runnable() {
			public void run() {
				//ImageCapture.getInstance().capture();
			//	if (!ImageCapture.getInstance().saveToFile(null))
				//	GUIFacade.getInstance().showDialog(
